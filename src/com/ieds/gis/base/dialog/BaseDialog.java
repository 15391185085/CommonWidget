...
public class BaseDialog extends Dialog implements
	...
	private boolean keepOnClickVisible;
	...
	public BaseDialog(Activity act) {
		this(act, R.layout.common_dialog_generic);
		initViews();
		initEvents();
		setCancelable(true);
	}
	 ...
	 * @param context
	 */
	public void addView(View context) {
		LinearLayout addLL = (LinearLayout) this.findViewById(R.id.addLayout);
		if (addLL != null) {
			addLL.addView(context);
		}
	}

	protected BaseDialog(Activity context, int layout) {
	...
	public static BaseDialog getDialog(Activity context, int titleId,
	...
	public void show() {
		...
		if (BaseActivity.getQueue().contains(mContext)) {
			// 判断存活
			super.show();
		} else {
			this.dismiss();
	...
	/**
	 * 点击后对话框是否还显示，默认 false：不显示
	 * 
	 * @param isOnClickClose
	 */
	public void setKeepOnClickVisible(boolean keepOnClickVisible) {
		this.keepOnClickVisible = keepOnClickVisible;
	}

	@Override
	public void onClick(View v) {
		if (!keepOnClickVisible) {
			this.dismiss();
		}
		if (v.getId() == R.id.dialog_generic_btn_button1) {
			if (mOnClickListener1 != null) {
				mOnClickListener1.onClick(mBaseDialog,
						DialogInterface.BUTTON_POSITIVE);
			}
		} else if (v.getId() == R.id.dialog_generic_btn_button2) {
			if (mOnClickListener2 != null) {
				mOnClickListener2.onClick(mBaseDialog,
						DialogInterface.BUTTON_NEGATIVE);
			}
		} else if (v.getId() == R.id.dialog_generic_btn_button3) {
			if (mOnClickListener3 != null) {
				mOnClickListener3.onClick(mBaseDialog,
