...
public class PageUtil {
	...
	public static final int LINE_SIGN = 8; // 换行判断的标准
	public static final int MAX_LINE_SIGN = 5; // 超过标准时换行的位数
	...
	public static String getShortName(String name) {
		...
		if (len < LINE_SIGN) {
			...
			for (int i = len; i > 0; i -= MAX_LINE_SIGN) {
				if (i <= MAX_LINE_SIGN) {
					sb.append(name.substring(len - i, len));
				} else {
					sb.append(name.substring(len - i, len - i + MAX_LINE_SIGN)
							+ "\n");
				}
			}
		}
	...
	public static String getSessionTime(int mTimeStamp) {
		...
		/**
		 * 今天的显示 hh:mm (今天星期三) 昨天 星期一 星期日 、 星期六、 星期五、星期四 yy-hh-mm
		 * */
		do {
	...
	public static String parseTime(long longTime) {
		...
		String strTime = sdf.format(longTime * 1000);
		return strTime;
	}
